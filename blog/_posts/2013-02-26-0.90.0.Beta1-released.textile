...
p. The suggest feature (part of a search request) allows Elasticsearch to provide suggestions for a given text based on the corpus that is part of the index. The current implemented suggest type uses the Levenshtein distance on a per term basis to potentially provide spelling suggestions.
