...
p. ElasticSearch provides just that with a module called "Gateway":/guide/reference/modules/gateway/. For example, to have the state  stored in a shared file system, each node needs to start with a configuration of:
