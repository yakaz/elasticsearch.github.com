...
p. *elasticsearch* configuration files can be found under @ES_HOME/config@ folder. The folder comes with two files, the @elasticsearch.yml@ for configuring ElasticSearch different "modules":/guide/reference/modules, and @logging.yml@ for configuring the ElasticSearch logging.
...
p. Another option is to set @es.default.@ prefix instead of @es.@ prefix, which means the default setting will be used only if not explicitly set in the configuration file.
