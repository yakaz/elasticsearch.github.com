...
p. The store module allow you to control how index data is stored.
...
p. When using a local gateway (the default), file system storage with *no* in memory storage is required to maintain index consistency. This is required since the local gateway constructs its state from the local index state of each node.

p. Another important aspect of memory based storage is the fact that ElasticSearch supports storing the index in memory *outside of the JVM heap space* using the "Memory" (see below) storage type. It translates to the fact that there is no need for extra large JVM heaps (with their own consequences) for storing the index in memory.

h1. Store Level Compression

p. **From version 0.90 onwards, store compression is always enabled.**

p. For versions 0.19.5 to 0.20:

p. In the mapping, one can configure the @_source@ field to be compressed. The problem with it is the fact that small documents don't end up compressing well, as several documents compressed in a single compression "block" will provide a considerable better compression ratio. This version introduces the ability to compress stored fieds using the @index.store.compress.stored@ setting, as well as term vector using the @index.store.compress.tv@ setting.
...
p. File system based storage is the default storage used. There are different implementations or storage types. The best one for the operating environment will be automatically chosen: @mmapfs@ on Solaris/Windows 64bit, @simplefs@ on Windows 32bit, and @niofs@ for the rest.

p. The following are the different file system based storage types:

h2. Simple FS

p. The @simplefs@ type is a straightforward implementation of file system storage (maps to Lucene @SimpleFsDirectory@) using a random access file. This implementation has poor concurrent performance (multiple threads will bottleneck). Its usually better to use the @niofs@ when you need index persistence.

h2. NIO FS

p. The @niofs@ type stores the shard index on the file system (maps to Lucene @NIOFSDirectory@) using NIO. It allows multiple threads to read from the same file concurrently. It is not recommended on Windows because of a bug in the SUN Java implementation.
...
p. The @memory@ type stores the index in main memory with the following configuration options:
