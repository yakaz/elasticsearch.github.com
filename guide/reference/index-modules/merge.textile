...
p. Lucene indices are produced in segments over time.  The more segments, the slower a search will be.  Merging segments speeds up searching, but merging is an expensive operation itself. The @Merge@ module allows you to set the merging policy. 
...
p. Merge schedulers controls the execution of merge operations once they are needed (according to the merge policy). The following types are support with the default set to @ConcurrentMergeScheduler@.
