...
p. Shard level slow search log allows to log slow search (query and fetch executions) into a dedicated log file.

p. Thresholds can be set for both the query phase of the execution, and fetch phase, here is a sample:
...
p. All settings are index level settings (and each index can have different values for it), and can be changed in runtime using the index update settings API.

p. The logging file is configured by default using the following configuration (found in @logging.yml@):
...
p. The index slow log file is configured by default in the @logging.yml@ file:
