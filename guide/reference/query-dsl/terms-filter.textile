...
p. In the case when specifying a @terms@ filter with a lot of terms it can beneficial to fetch those term values from a document in an index. A concrete example would be to filter tweets tweeted by your followers. Potenially the amount of user ids specified in the terms filter can be a lot. In this scenerio it makes sense to use the terms filter's terms lookup mechanism.
...
p. There is an additional cache involved, which caches the lookup of the lookup document to the actual terms. It is by default set to @10mb@ LRU size, but can be explicitly set using @indices.cache.filter.terms.size@. On top of this the lookup of the actual terms can also optionally be expired after the last write or last read in a time based manner. To expire after the last read use the @indices.cache.filter.terms.expire_after_access@ option or the expire after the last write use the @indices.cache.filter.terms.expire_after_write@ option. Both are disabled by default. All options for the lookup of the documents cache can only be configured via the @elasticsearch.yml@ file
...
The above is higly optimized, both in a sense that the list of followers will not be fetched if the filter is already cached in the filter cache, and with internal LRU cache for fetching external values for the terms filter. Also, the entry in the filter cache will not hold @all@ the terms reducing the memory required for it.
