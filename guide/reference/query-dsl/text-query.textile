...
p. A new family of @text@ queries that accept text, analyzes it, and constructs a query out of it. For example:
...
p. It accepts the same parameters as the phrase type. In addition, it also accepts a @max_expansions@ parameter that can control to how many prefixes the last term will be expanded. It is highly recommended to set it to an acceptable value to control the execution time of the query.
