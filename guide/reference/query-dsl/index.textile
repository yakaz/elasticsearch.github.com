...
p. Think of the Query DSL as an AST of queries. Certain queries can contain other queries (like the "bool":bool-query.html query), other can contain filters (like the "constant_score":constant-score-query.html), and some can contain both a query and a filter (like the "filtered":filtered-query.html). Each of those can container *any* query of the list of queries or *any* filter from the list of filters, resulting in the ability to build quite complex (and interesting) queries.
