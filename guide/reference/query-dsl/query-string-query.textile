...
p. When a multi term query is being generated, one can control how it gets rewritten using the "rewrite":multi-term-rewrite.html parameter.
...
p. So, if @_all@ field is disabled, it might make sense to change it to set a different default field.a
...
p. Another option is to provide the wildcard fields search in the query string itself (properly escapign the @*@ sign), for example: @city.\*:something@. (since 0.19.4).
