...
p. Creating new mappings can be done using the "Put Mapping":/guide/reference/api/admin-indices-put-mapping.html API. When a document is indexed with no mapping associated with it in the specific index, the "dynamic / default mapping":/guide/reference/mapping/dynamic-mapping.html feature will kick in and automatically create mapping definition for it.
