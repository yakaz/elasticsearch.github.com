...
p. JSON documents are hierarchal in nature, allowing to define inner "object"s within the actual JSON. ElasticSearch completely understand the nature of objects and allows to map them easily, automatically support their dynamic nature (one object can have different fields each time), and provides query support for their inner field. Let's take the following JSON as an example:
...
p. The above shows an example where a tweet includes the actual @person@ details. A @person@ is an object, with an @sid@, and a @name@ object which has @first_name@ and @last_name@. It's important to note that @tweet@ is also an object, though a special "root object type":/guide/reference/mapping/root-object-type.html which allows for additional mapping definitions.
...
p. An object mapping can optionally define one or more properties using the @properties@ tag. Properties list the properties this field will have. Each property can be either another @object@, or one of the "core_types":/guide/reference/mapping/core-types.html.

h1. dynamic

p. One of the most important features of ElasticSearch is its ability to be schema-less. This means that, in our example above, the @person@ object can later on be indexed with a new property, for example, @age@, and it will automatically be added to the mapping definitions. Same goes for the @tweet@ root object.

p. This feature is by default turned on, and it's the @dynamic@ nature of each object mapped. Each object mapped is automatically dynamic, though it can be explicitly turned off:
...
p. When processing dynamic new fields, their type is automatically derived. For example, if it is a @number@, it will automatically be treated as number "core_type":/guide/reference/mapping/core-types.html. Dynamic fields default to their default attributes, for example, they are not stored and they are always indexed.
...
p. The @dynamic@ parameter can also be set to @strict@, meaning that not only new fields will not be introduced into the mapping, parsing (indexing) docs with such new fields will fail.
...
|_. JSON Name |_. Document Field Name |
|@name1@/@first1@|@first1@|
