...
p. An object mapping can optionally define one or more properties using the @properties@ tag. Properties list the properties this field will have. Each property can be either another @object@, or one of the "core_types":core-types.html.
...
p. When processing dynamic new fields, their type is automatically derived. For example, if it is a @number@, it will automatically be treated as number "core_type":core-types.html. Dynamic fields default to their default attributes, for example, they are not stored and they are always indexed.
