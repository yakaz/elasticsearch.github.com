...
p. The date type is a special type which maps to JSON string type. It follows a specific format that can be explicitly set. All dates are @UTC@. as. Internally, a date maps to a number type @long@, with the added parsing stage from string to long and from long to string. An example mapping:
