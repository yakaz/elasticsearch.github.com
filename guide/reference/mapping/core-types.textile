...
|@index_options@|Allows to set the indexing options, possible values are @docs@ (only doc numbers are indexed), @freqs@ (doc numbers and term frequencies), and @positions@ (doc numbers, term frequencies and positions). Defaults to @freqs@ for @analyzed@ fields, and to @docs@ for @not_analyzed@ fields. Since 0.20.|
...
p. Posting formats define how fields are encoded into the index and how fields are represented into memory. Posting formats can be defined per field via the @postings_format@ option. Postings format are configurable since version @0.90.0.Beta1@. Elasticsearch has several builtin formats:
...
h3. Defining custom postings formats

p. On top of using the built-in posting formats it is possible define custom postings format and reconfigure the default available postings formats. This can be specified via the index settings in the @codec@ settings part. This is considered an expert feature and most likely not needed for your indices.
...
p. Additionally, functionality for configuring the following Similarities is provided: @DFR@ and @IB@

h3. Configuring Similarity Properties

p. Most existing or custom Similarities have configuration options which can be configured.  This is done via index settings as shown below:
...
p. Here we configure the DFRSimilarity so it can be referenced as `my_similarity` in mappings.

h3. Default and Base Similarities
