...
p. The @multi_field@ type allows to map several "core_types":core-types.html of the same value. This can come very handy, for example, when wanting to map a @string@ type, once when it's @analyzed@ and once when it's @not_analyzed@. For example:
