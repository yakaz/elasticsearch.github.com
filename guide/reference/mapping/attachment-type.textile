...
p. The @attachment@ type allows to index different "attachment" type field (encoded as @base64@), for example, microsoft office formats, open document formats, ePub, HTML, and so on (full list can be found "here":http://lucene.apache.org/tika/0.8/formats.html).
