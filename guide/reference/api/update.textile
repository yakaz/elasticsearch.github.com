...
p. The update API also support passing a partial document (since 0.20), which will me merged into the existing document (simple recursive merge, inner merging of objects, replacing core "keys/values" and arrays). For example:
...
p. Last, it also support @upsert@ (since 0.20), when if the document does not already exists, the content of the @upsert@ element will be used to index the fresh doc:
