...
p. A @scroll_id@ is returned from the first search request (and from continuous) scroll requests. The @scroll_id@ should be used when scrolling (along with the @scroll@ parameter, to stop the scroll from expiring). The scroll id can also be passed as part of the search request body. Note that the @scroll_id@ changes for each scroll request and only the most recent one should be used.
...
p. For more information on scrolling, see the *scan* search type.
