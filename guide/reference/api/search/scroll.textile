...
p. A @scroll_id@ is returned from the first search request (and from continuous) scroll requests. The @scroll_id@ should be used when scrolling (and that's it, no other parameter is required). The scroll id can also be passed as part of the search request body. Note that the @scroll_id@ changes for each scroll request and only the most recent one should be used.
