...
p. The search API allows to execute a search query and get back search hits that match the query. It can be executed across "indices and types":/guide/reference/api/search/indices-types.html. The query can either be provided using a simple "query string as a parameter":/guide/reference/api/search/uri-request.html, or using a "request body":/guide/reference/api/search/request-body.html.
