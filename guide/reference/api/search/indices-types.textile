...
p. The search API can be applied to multiple types within an index, and across multiple indices with support for the "multi index syntax":/guide/reference/api/multi-index.html. For example, we can search on all documents across all types within the twitter index:
