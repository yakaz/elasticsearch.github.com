...
The possible actions are @index@, @create@ and @delete@.  @index@ and @create@ expect a source on the next line, and have the same semantics as the @op_type@ parameter to the standard index API (ie, create will fail if a document with the same index and type exists already).  @delete@ does not expect a source on the following line, and has the same semantics as the standard delete API.
