...
p. The above second curl example shows how an index called @twitter@ can be created with specific settings for it using "YAML":http://www.yaml.org. In this case, creating an index with 3 shards, each with 2 replicas. The index settings can also be defined with "JSON":http://www.json.org:
