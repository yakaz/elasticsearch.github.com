...
p. The API simply results in executing a search request with "moreLikeThis":../query_dsl/more_like_this_query query (http parameters match the parameters to the @more_like_this@ query). This means that the body of the request can optionally include all the request body options in the "search API":../search (facets, from/to and so on).
