...
**The S3 gateway is deprecated and will be removed in a future version. Please use the "local gateway":/guide/reference/modules/gateway/local.html instead.**
...
p. The @gateway.s3.concurrent_streams@ allow to throttle the number of streams (per node) opened against the shared gateway performing the snapshot operation. It defaults to @5@.
