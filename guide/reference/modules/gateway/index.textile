...
p. The gateway set on the node level will automatically control the index gateway that will be used. For example, if the @fs@ gateway is used, then automatically, each index created on the node will also use its own respective index level @fs@ gateway. In this case, if in an index should not persist its state, it should be explicitly set to @none@ (which is the only other value it can be set to).
