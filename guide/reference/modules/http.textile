...
p. It also shares the uses the common "network settings":/guide/clients/groovy-api/network.html.

h1. Disable HTTP

p. The http module can be completely disabled and not started by setting @http.enabled@ to @false@. This make sense when creating non "data nodes":/guide/clients/groovy-api/node.html which accept HTTP requests, and communicate with data nodes using the internal "transport":/guide/clients/groovy-api/transport.html.
