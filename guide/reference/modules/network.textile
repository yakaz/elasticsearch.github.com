...
p. There are several modules within a Node that use network based configuration, for example, the "transport":/guide/reference/modules/transport.html and "http":/guide/reference/modules/http.html modules. Node level network settings allows to set common settings that will be shared among all network based modules (unless explicitly overridden in each module).
...
p. When the @cloud-aws@ plugin is installed, the following are also allowed as valid network host settings:

|_. EC2 Host Value |_. Description |
