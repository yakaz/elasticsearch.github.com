...
p. The @include@ and @exclude@ values can have generic simple matching wildcards, for example, @value1*@. A special attribute name called @_ip@ can be used to match on node ip values. In addition @_host@ attribute can be used to match on either the node's hostname or its ip address.
