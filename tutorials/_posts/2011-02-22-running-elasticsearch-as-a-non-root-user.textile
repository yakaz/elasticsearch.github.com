...
* Create the user:
<pre>    useradd -d $ES_HOME -s /bin/sh elasticsearch</pre>
...
* Edit <code>/etc/security/limits.conf</code> and add this line (this increases the number of files that the <code>elasticsearch</code> user can open, see "Raising nofile limits":/tutorials/2011/04/06/too-many-open-files.html if you have any problem here):
