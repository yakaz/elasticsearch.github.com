...
p. Note: This tutorial applies to elasticsearch 0.18 and below. 0.19 has moved to use @mvn@ as the build system, which makes it simpler to import it easily to IDEs. Creating a packaged distribution is simple as running @mvn package -DskipTests@ with the distribution existing under @target/releases@ directory.
