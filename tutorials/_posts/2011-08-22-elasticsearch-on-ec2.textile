...
<p>
    ...
    that the local hard disk stores a copy of the cluster metadata, and the local
...
<p>
    Note: When using the local gateway option, you cannot use
    "memory caching":http://www.elasticsearch.org/guide/reference/index-modules/store.html  as your index store type. This would be a shame
    if you were running a cluster of EC2 high memory instances with capacities
    > 64GB per node!
</p>
...
<p>
    ...
    without the benefit of an EBS volume, or need the performance increase that
    comes with a high memory instance. For these cases, there is the S3 Gateway.
...
<p>
    ...
    but it would be a welcome addition to this document.
</p>
...
<p>
    ...
    "Sonatype’s repository":http://oss.sonatype.org/content/repositories/releases/ , not Maven central.
</p>

<p>
    If you are using the Java libraries directly by including them in your
    projects runtime classpath, the easiest way to get the cloud libraries is to
    download the jar file from an online archive stored on "googlecode":http://elasticsearch.googlecode.com/svn/plugins/cloud-aws/ .
</p>
...
<p>
    There has been some confusion regarding the verb “recover”.
    It is used in some places to represent data going into S3, 
    and coming out of S3. In this case, recover refers to
    the process of writing the current index data into S3.
</p>
